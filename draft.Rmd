...
coord$Language <- factor(coord$Language, levels = c("Chechen", "Avar", "Andi", "Botlikh", "Godoberi", "Karata"))
...
In order to measure how the answers of the speakers are uniform, two measures were used: the percentage of absolute percentage  and the Fleiss' Kappa. The characteristics of the obtained values of the Fleis' kappa were taken from the work [@hallgren12]. The results are shown in Table 3:
...
As we see from Table 3, the results of the experiment showed a fairly high inter speaker agreement in class assignment task. The agreement of the speakers in group of borrowed words is less (substantial agreement vs. almost perfect agreement). Although the agreement in the native words is quite high, but it is not ideal. This shows that variability in class assignment is observed in all parts of vocabulary, but in borrowings it is much higher.

Also we decided to make a hierarchical clustering on our data, the results of which are presented in Figure 3:

```{r, dpi = 500, cashe = TRUE, fig.cap= "Figure 3. Dendrogram showing the clusterization of speakers."}
df %>% 
  mutate(class = if_else(class == "b", 1, 0)) %>% 
  select(-sex, -age_2017, -group_age) %>% 
  spread(s_id, class) %>% 
...
On the dendrogram in the Figure 3 we could see 3 different clusters that is defirintiated by age group:

* older group from 34 to 60;
* younger group from 13 to 32;
* one sexteen year old female speaker form a separate cluster.

Since hierarchical clustering showes age group importance we decided recalculate all obtained measures for each age group: 
...
Table 4. Pure agreement percentages and Fleiss' kappas for different age group
```{r}
read_csv("data/table_4.csv", na = "")
```

As we see from Table 4, in each group of words younger speakers has lesser values of pure agreement percentages and Fless' kappas. It is possible to obtaine this values in two different situations. First, it is possible that lexems where we found variability are the same among two age group, but the level of variability is higher.  It is also possible that variable lexemes are differnt in different groups. We decided to show this visually. First we calculated variability rate for each word in each group. Here are three examples:

* the word *inɡur* 'window' were atributed as a first inanimated class by all speakers, so its variability rate will be $$\frac{inanimate_1}{inanimate_1+inanimate_2} = \frac{16}{8+8} = 1$$

* the word *haq'u* 'room' were atributed as a first inanimated class by all speakers, so its variability rate will be 
$$\frac{inanimate_1}{inanimate_1+inanimate_2} = \frac{0}{8+8} = 0$$

* the word *haq'u* 'room' were atributed as a first inanimated class by all speakers, so its variability rate will be 
$$\frac{inanimate_1}{inanimate_1+inanimate_2} = \frac{0}{8+8} = 0$$

* the word *haq'u* 'butterfly' were atributed as a first inanimated class by older speakers 4 times, and by younger speakers 5 times, so its variability rate will be different in different groups: $$\frac{inanimate_1}{inanimate_1+inanimate_2} = \frac{4}{4+4} = 0.5$$
$$\frac{inanimate_1}{inanimate_1+inanimate_2} = \frac{5}{5+3} = 0.625$$

On the Figure 4 the variability rate of younger speakers is plotted against variability rate of older speakers.

```{r, dpi = 500, cashe = TRUE, fig.cap= "Figure 4. Destribution of the most variable words by different age group.", fig.width=7}
df %>%  
